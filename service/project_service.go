...
func GetProjectCodeName(uuid string) (models.ProjectCodeName, error) {
	...
	err := db.Get(&projectCodeName, "SELECT project_uuid, project_code, project_name FROM project_ms WHERE project_uuid = $1", uuid)
	if err != nil {
...
func IsUniqueProject(uuid, code, name string) (bool, error) {
	...
	err := db.Get(&exsitingProCode, "SELECT project_code FROM project_ms WHERE project_uuid = $1", uuid)
	if err != nil && err != sql.ErrNoRows {
		return false, err
	}

	err = db.Get(&exsitingProName, "SELECT project_name FROM project_ms WHERE project_uuid = $1", uuid)
	if err != nil && err != sql.ErrNoRows {
		return false, err
	}
